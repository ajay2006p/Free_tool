    ...
    <div>
      ...
      <div className="grid grid-2" style={{ marginBottom: 20 }}>
        <div className="card">
          ...
          <p className="muted" style={{ fontSize: 13, margin: 0 }}>≈ {s.totalViews.toLocaleString()} views × ${s.earnings.cpm} CPM ÷ 1000. Set <code>ADSTERRA_CPM</code> in admin/.env.</p>
        </div>
        <div className="card">
          ...
          {s.earnings.connected ? (
            ...
            <p className="muted" style={{ fontSize: 13, marginTop: 8 }}>Add <code>ADSTERRA_API_TOKEN</code> in admin/.env (Adsterra → Settings → API) to pull real revenue. {s.earnings.error ? `(${s.earnings.error})` : ""}</p>
          )}
        </div>
      </div>
