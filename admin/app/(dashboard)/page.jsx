...
export default async function Dashboard() {
  ...
  return (
    <div className="container">
      ...
      {error ? (
        <div className="notice notice-error">Database error: {error}. Make sure the shared DB exists — run <code>npm run setup</code> in the <code>web</code> folder first.</div>
      ) : null}
      ...
      <div className="flex-between" style={{ marginBottom: 14 }}>
        ...
        <Link href="/posts/new" className="btn btn-sm btn-outline">+ Add</Link>
      </div>

      <div className="sheet" style={{ padding: 8 }}>
        {posts.length === 0 ? (
          <div className="empty">No posts yet. <Link href="/posts/new">Write your first post →</Link></div>
        ) : (
              ...
              <tbody>
                {posts.map((p) => (
                  <tr key={p.id}>
                    ...
                    <td><div className="row-actions"><Link href={`/posts/edit/${p.id}`} className="btn btn-sm btn-outline">Edit</Link></div></td>
                  </tr>
                ))}
              </tbody>
            </table>
